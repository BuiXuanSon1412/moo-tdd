...
def relaxed_repair(chromosome, problem: Problem):
    ...
    while True:
        ...
        def opt_pair_cust(trip):
            ...
            opt_lch_wait = None
            opt_ld_wait = None
            if sum([problem.customer_list[cust].quantity for cus in trip]) > problem.drone_capacity:
            ...
            for pair in pairs:
                ...
                lch_cust = pair[0]
                ld_cust = pair[1]
                if problem.customer_list[lch_cust].arrive_time > problem.customer_list[ld_cust].arrive_time:
                    lch_cust, ld_cust = ld_cust, lch_cust
                
                trip = [lch_cust] + trip + [ld_cust]
                received = [0] + [problem.customer_list[cust].quantity for cust in trip] + [0]
                ...
                if truck_schedule[lch_cust][0] <= problem.customer_list[trip[1]].arrive_time - time_by_drone[lch_cust][trip[1]] <= truck_schedule[lch_cust][1]:
                    leave[0] = problem.customer_list[lch_cust].arrive_time - time_by_drone[lch_cust][trip[1]]
                    lch_wait = 0
                elif problem.customer_list[trip[1]].arrive_time - time_by_drone[lch_cust][trip[1]] > truck_schedule[lch_cust][1]:
                    leave[0] = problem.customer_list[lch_cust].arrive_time - time_by_drone[lch_cust][trip[1]]
                    lch_wait = leave[0] - truck_schedule[lch_cust][1]
                    #truck_schedule[lch_cust][1] = leave[0]
                else:
                ...
                for i in range(len(trip) - 1):
                    ...
                    total_energy = total_energy + problem.energy_consumption_rate*(problem.weight_of_drone + total_goods)*(problem.distance_matrix_drone[u][u_next]/problem.speed_of_drone + problem.launch_time)
                    if received[i+1] != 0:
                        arrive[i+1] = max(problem.customer_list[u_next].arrive_time, leave[i] + time_by_drone[u][u_next])
                        wait_time = max(problem.customer_list[u_next].arrive_time - arrive[i+1], 0)
                        leave[i+1] = arrive[i+1] + problem.customer_list[u_next].service_time + wait_time
                        ...
                        ld_wait = wait_time
                        total_energy = total_energy + problem.energy_consumption_rate*(problem.weight_of_drone + total_goods)*wait_time
                if total_energy <= problem.drone_energy:
                    if (opt_lch_wait is None or opt_ld_wait is None) or (lch_wait + ld_wait < opt_lch_wait + opt_ld_wait):
        ...
        drone_schedule = [[[0, 0] for _ in range(problem.number_of_drones)] for _ in range(len(problem.customer_list) + 1)]
        drone_triplists = []
        violated = False
        for drone_idx, drone_route in enumerate(drone_routes):
            ...
            while drone_route:
                trip = []
                opt_lch_cust, opt_ld_cust, opt_arrive, opt_leave = None, None, None, None
                ...
                for idx, cust in trip:
                    drone_schedule[cust][drone_idx][0] = opt_arrive[idx]
                    drone_schedule[cust][drone_idx][1] = opt_leave[idx]      
...
def schedule(chromosome, truck_routes, drone_triplists, problem: Problem):
    ...
    return truck_routes, drone_triplists, truck_arrival, truck_depart, drone_arrival, drone_depart

def build_solution(truck_routes, drone_triplists, truck_arrival, truck_depart, drone_arrival, drone_depart, problem):
    return_depot = len(problem.customer_list) + 1
    ld_custs = [
        trip[-1]
        for trips in drone_triplists
...
def decode(individual: Individual, problem: Problem):
    ...
    while True:
        ...
        truck_routes, drone_triplists, truck_arrival, truck_depart, drone_arrival, drone_depart = schedule(
        ...
        if truck_routes:
            ...
            return build_solution(truck_routes, drone_triplists, truck_arrival, truck_depart, drone_arrival, drone_depart, problem)
