...
def repair_capacity(chromosome, problem: Problem):
    ...
    while True:
        ...
        if max_load > problem.truck_capacity:
            max_cust = max(
                truck_routes[max_idx], key=lambda c: problem.customer_list[c].quantity
            )
            truck_routes[max_idx].remove(max_cust)
            truck_routes[min_idx].append(max_cust)
        else:
...
def repair_distance(chromosome, problem: Problem):
    max_distance = (
        (
            problem.drone_energy
            / (problem.energy_consumption_rate * problem.weight_of_drone)
        )
        - problem.land_time
        - problem.launch_time
...
def find_nearest_customer(
    customer_drone_list, customer_truck_list, distance_drone_matrix
):
    nearest_dict = {}
    ...
    for customer in customer_drone_list:
        distances = [
            ...
            for other in customer_truck_list
            if other != customer
        ]
...
def init_truck_solution(route, problem: Problem):
    ...
    recived_drone[0] = 0
    leave_time[0] = (
        max(arrive_time[0], problem.customer_list[route[0]].arrive_time)
        + problem.customer_list[route[0]].service_time
    )
    for i in range(1, num_point):
        arrive_time[i] = (
            leave_time[i - 1]
            + problem.distance_matrix_truck[route[i - 1]][route[i]]
            / problem.speed_of_truck
        )
        recived_truck[i] = problem.customer_list[route[i]].quantity
        recived_drone[i] = 0
        leave_time[i] = (
            max(arrive_time[i], problem.customer_list[route[i]].arrive_time)
            + problem.customer_list[route[i]].service_time
        )
    truck_solution = Truck_Solution(
        assigned_customer, recived_truck, recived_drone, arrive_time, leave_time
    )
    return truck_solution


def update_truck_solution(
    truck_solution: Truck_Solution,
    problem: Problem,
    customer,
    add_quantity,
    ...
    if customer in truck_solution.assigned_customers:
        ...
        truck_solution.leave_time[idx] = max(
            truck_solution.leave_time[idx], leave_arrive
        )
        if truck_solution.leave_time[idx] == leave_arrive:
            ...
            for i in range(idx + 1, len(truck_solution.assigned_customers)):
                truck_solution.arrive_time[i] = (
                    truck_solution.leave_time[i - 1]
                    + problem.distance_matrix_truck[route[i - 1]][route[i]]
                    / problem.speed_of_truck
                )
                truck_solution.leave_time[i] = (
                    max(
                        truck_solution.arrive_time[i],
    ...
    truck_solution.recived_truck = np.insert(
        truck_solution.recived_truck, index_insert, 0
    )
    truck_solution.recived_drone = np.insert(
        truck_solution.recived_drone, index_insert, add_quantity
    )
    ...
    if index_insert == 0:
        truck_solution.arrive_time[0] = (
            problem.distance_matrix_truck[0][route[0]] / problem.speed_of_truck
        )
    else:
        truck_solution.arrive_time[index_insert] = (
            truck_solution.leave_time[index_insert - 1]
            + problem.distance_matrix_truck[route[index_insert - 1]][
                route[index_insert]
            ]
            / problem.speed_of_truck
        )

    truck_solution.leave_time[index_insert] = max(
        max(
            truck_solution.arrive_time[index_insert],
            problem.customer_list[route[index_insert]].arrive_time,
        )
        + problem.customer_list[route[index_insert]].service_time,
        leave_arrive,
    )
...
def find_solution(
    truck_routes: List[List[int]], drone_trips: List[List[int]], problem: Problem
):
    ...
    for trip in drone_trips:
        ...
        customer_on_trip = [
            (customer, problem.customer_list[customer].arrive_time) for customer in trip
        ]
        ...
        while unserved:
            ...
            arrive_time_request = problem.customer_list[
                first_unserved_customer
            ].arrive_time
            ucv_launch = []
            ...
            if current_customer == 0:
                drone_fly_duration = (
                    problem.launch_time
                    + problem.distance_matrix_drone[0][first_unserved_customer]
                    / problem.speed_of_drone
                    + problem.land_time
                )
                drone_launch_time = max(
                    current_system_time, arrive_time_request - drone_fly_duration
                )
                # print(drone_launch_time)
                depot_consumption_energy = (
                    problem.energy_consumption_rate
                    * (problem.weight_of_drone)
                    * drone_fly_duration
                    + problem.energy_consumption_rate
                    * (
                        problem.weight_of_drone
                        + problem.customer_list[first_unserved_customer].quantity
                    )
                    * problem.customer_list[first_unserved_customer].service_time
                )
                # print(depot_consumption_energy)
                drone_arrive_to_customer_time = drone_launch_time + drone_fly_duration

                customer_wait_time = (
                    drone_arrive_to_customer_time
                    - problem.customer_list[first_unserved_customer].arrive_time
                )
                ...
                for truck_sol in truck_solutions:
                    for pos, cus in enumerate(truck_sol.assigned_customers):
                        drone_fly_duration = (
                            problem.launch_time
                            + problem.distance_matrix_drone[cus][
                                first_unserved_customer
                            ]
                            / problem.speed_of_drone
                            + problem.land_time
                        ...
                        if drone_launch_time <= truck_sol.leave_time[pos]:
                            ...
                            drone_arrive_to_customer_time = (
                                drone_launch_time + drone_fly_duration
                            )
                            customer_wait_time = (
                                drone_arrive_to_customer_time
                                - problem.customer_list[
            ...
            for launch_node_candidate_tup in ucv_launch:
                best_assigned_customer = [
                    launch_node_candidate_tup[0],
                    launch_node_candidate_tup[-1],
                ]
                best_arrive_time = [
                    launch_node_candidate_tup[2],
                    launch_node_candidate_tup[4],
                ]
                best_leave_time = [
                    launch_node_candidate_tup[3],
                    launch_node_candidate_tup[5],
                ]
                best_recive_done = [
                    0,
                    problem.customer_list[launch_node_candidate_tup[-1]].quantity,
                ]
                ...
                drone_fly_duration = (
                    problem.launch_time
                    + problem.land_time
                    + problem.distance_matrix_drone[current_cus][0]
                    / problem.speed_of_drone
                )
                energy_fly_to_depot = (
                    problem.energy_consumption_rate
                    * (problem.weight_of_drone + current_quantity)
                    * drone_fly_duration
                )
                if (
                    energy_fly_to_depot + current_consumption_energey
                    <= problem.drone_energy
                ):
                    if len(unserved) > 1:
                ...
                for truck_sol in truck_solutions:
                    if (
                        np.sum(truck_sol.recived_truck)
                        + np.sum(truck_sol.recived_drone)
                        + sum(best_recive_done)
                    ...
                    for pos, cus in enumerate(truck_sol.assigned_customers):
                        drone_fly_duration = (
                            problem.launch_time
                            + problem.land_time
                            + problem.distance_matrix_drone[current_cus][cus]
                            / problem.speed_of_drone
                        )
                        energy_fly_to_land_point = (
                            problem.energy_consumption_rate
                            * (problem.weight_of_drone + current_quantity)
                            * drone_fly_duration
                        )
                        ...
                        if (
                            energy_fly_to_land_point + current_consumption_energey
                            <= problem.drone_energy
                            and drone_land_time >= truck_sol.arrive_time[pos]
                            and drone_land_time <= truck_sol.leave_time[pos]
                        ):
                            if len(unserved) > 1:
                                next_distance = problem.distance_matrix_drone[cus][
                                    unserved[1]
                                ]
                            else:
                                next_distance = 0
                ...
                for i in range(2, len(unserved) + 1):
                    ...
                    temp_leave_time = []
                    temp_recive_drone = []
                    total_capacity_drone = problem.customer_list[
                    ...
                    for iii in range(1, len(customers_to_serve)):
                        ...
                        drone_arrive_time = (
                            ready_time
                            + problem.launch_time
                            + problem.distance_matrix_drone[
                                customers_to_serve[iii - 1]
                            ][cus]
                            / problem.speed_of_drone
                            + problem.land_time
                        )
                        drone_wait_duration = max(
                            0,
                            problem.customer_list[cus].arrive_time - drone_arrive_time,
                        )
                        consumption_energy_new = (
                            consumption_energy_new
                            + problem.energy_consumption_rate
                            * (problem.weight_of_drone + total_capacity_drone)
                            * (
                                problem.launch_time
                                + problem.distance_matrix_drone[
                                    customers_to_serve[iii - 1]
                            ...
                            * (
                                problem.weight_of_drone
                                + total_capacity_drone
                                + problem.customer_list[cus].quantity
                            )
                            * (problem.customer_list[cus].service_time)
                        )
                        total_capacity_drone = (
                            total_capacity_drone + problem.customer_list[cus].quantity
                        )
                        ready_time = (
                            ready_time
                            + problem.launch_time
                    ...
                    if (
                        total_capacity_drone > problem.drone_capacity
                        or consumption_energy_new + current_consumption_energey
                        > problem.drone_energy
                    ):
                        break
                    else:
                        ...
                        energy_fly_to_depot = (
                            ...
                            * (
                                problem.launch_time
                                + problem.land_time
                                + problem.distance_matrix_drone[customers_to_serve[-1]][
                                    0
                                ]
                                / problem.speed_of_drone
                            )
                        ...
                        if (
                            ...
                            if i == len(unserved):
                                ...
                                next_distance = problem.distance_matrix_drone[0][
                                    unserved[i]
                                ]
                            land_drone = (
                                ready_time
                            ...
                            for pos, cus in enumerate(truck_sol.assigned_customers):
                                ...
                                if (
                                    ...
                                    if i == len(unserved):
                                        ...
                                        next_distance = problem.distance_matrix_drone[
                                            cus
                                        ][unserved[i]]
                                    ucv_land.append(
                                        (cus, next_distance, drone_land_time)
                                    )
                        ucv_land.sort(key=lambda x: x[1])
                        if len(ucv_land) != 0:
                ...
                if temp_best_assigned != None:
                    if best_recive_done[-1] != 0:
                        best_assigned_customer = (
                            best_assigned_customer + temp_best_assigned[1:]
                        )
                        ...
                        best_assigned_customer = (
                            best_assigned_customer[:-1] + temp_best_assigned[1:]
                        )
                    ...
                    if cus_update != 0:
                        ...
                        for idx, truck_sol in enumerate(truck_solutions):
                            if cus_update in truck_sol.assigned_customers:
                                idx_truck = idx
                                break
                        truck_solutions[idx_truck] = update_truck_solution(
                            truck_solutions[idx_truck],
                            problem,
                            cus_update,
                            add_good,
                            leave_time_update,
